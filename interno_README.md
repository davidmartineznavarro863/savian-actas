...
Savian: **Tickets internos**, **JORNADA**, **Savian Guardian**, **Portal
Savian** y **SavianDM**. Cada herramienta tiene, dentro del hub, dos
botones:
...
```
interno_savian_apps/
├── interno_index.html          → pantalla del hub (las tarjetas, Abrir/Descargar)
...
│   ├── interno_saviandm/         → mini-lanzador instalable de SavianDM
...
## Sobre el tamaño de los logos

No hace falta que un logo venga en 512×512 exactos: el script
`interno_generar_iconos.py` (y el proceso con el que se generaron estos
iconos) redimensiona cualquier imagen de origen a los tamaños que hacen
falta (192, 512, 180, 64, 32 px). Lo único que sí importa es que el logo
de origen sea **cuadrado** (misma anchura que altura) — si no lo es, se
deforma al redimensionar. Los logos de Tickets internos y JORNADA que
enviaste, por ejemplo, venían en 1254×1254 (cuadrados, pero no 512): se
han usado igualmente sin ningún problema porque el cuadrado se conserva.
Si algún día envías un logo rectangular, avísame y lo recorto a cuadrado
antes de generar los iconos.
