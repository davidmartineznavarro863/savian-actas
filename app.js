...
function validarFormulario() {
  ...
  for (const id of obligatorios) {
    ...
    if (!campo || !campo.value.trim()) {
      ...
      alert('Completa todos los datos generales obligatorios');
      return false;
  ...
  for (const trabajador of trabajadores) {
    ...
    if (tieneAlgo) {
      ...
      if (!entrada.value) {
        entrada.style.borderColor = '#dc2626';
        entrada.focus();
      ...
      if (!salida.value) {
        salida.style.borderColor = '#dc2626';
        salida.focus();
  ...
  if (!trabajadorValido) {
    alert('Debes añadir mínimo un trabajador completo');
    return false;
  }
  ...
  if (!tipoActivo) {
    alert('Debes seleccionar si es visita o incidencia');
    return false;
  }

  // FIRMAS
  const firmaTecnico = document.getElementById('firma1');
  const firmaCliente = document.getElementById('firma2');

  const firmaTecnicoVacia =
    firmaTecnico.toDataURL() === blankCanvas(firmaTecnico);

  const firmaClienteVacia =
    firmaCliente.toDataURL() === blankCanvas(firmaCliente);

  if (firmaTecnicoVacia) {
    alert('La firma del técnico es obligatoria');
    return false;
  }

  if (firmaClienteVacia) {
