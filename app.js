  ...
  row.innerHTML = `
    ...
    <div class="worker-inner">
      ...
      <div class="worker-times">
        <div>
          <label style="font-size:10px">Entrada <span class="required">*</span></label>
          <input type="time" style="font-size:16px;padding:10px 8px">
        </div>
        <div>
          <label style="font-size:10px">Salida <span class="required">*</span></label>
          <input type="time" style="font-size:16px;padding:10px 8px">
        </div>
...
function avisoValidacion(mensaje, campo = null) {
  limpiarErroresValidacion();
  marcarError(campo);
  if (typeof showToast === 'function') {
    showToast('⚠️ ' + mensaje, 'warn');
  } else {
    alert(mensaje);
  }
}
...
function validarFormulario() {
  ...
  // DATOS GENERALES: localización específica NO es obligatoria.
  const obligatorios = [
    { id: 'fecha', msg: 'La fecha es obligatoria' },
    { id: 'cliente', msg: 'El cliente es obligatorio' },
    { id: 'centro', msg: 'El centro de trabajo es obligatorio' },
    { id: 'responsableFirma', msg: 'El responsable de la empresa que firma es obligatorio' }
  ];

  for (const item of obligatorios) {
    const campo = document.getElementById(item.id);
    if (!campo || !campo.value.trim()) {
      avisoValidacion(item.msg, campo);
      return false;
    }
  }

  // TRABAJADORES: mínimo uno y, si se empieza una fila, nombre + entrada + salida.
  const trabajadores = document.querySelectorAll('.worker-row');
  let trabajadorValido = false;

  for (const trabajador of trabajadores) {
    const nombre = trabajador.querySelector('.worker-nombre');
    const horas = trabajador.querySelectorAll('input[type="time"]');
    const entrada = horas[0];
    const salida = horas[1];
    ...
    if (tieneAlgo) {
      if (!nombre?.value.trim()) {
        avisoValidacion('El nombre del trabajador es obligatorio', nombre);
        return false;
      }
      if (!entrada?.value) {
        avisoValidacion('La hora de entrada del trabajador es obligatoria', entrada);
        return false;
      }
      if (!salida?.value) {
        avisoValidacion('La hora de salida del trabajador es obligatoria', salida);
        return false;
      }
  ...
  if (!trabajadorValido) {
    const primerNombre = document.querySelector('.worker-nombre');
    avisoValidacion('Debes añadir mínimo un trabajador con nombre, entrada y salida', primerNombre);
    return false;
  }

  // TIPO DE TRABAJO
  if (!tipoActivo) {
    avisoValidacion('Debes seleccionar si es visita o incidencia', document.getElementById('card-tipo'));
    return false;
  }

  // FIRMAS
  if (!fS[1]) {
    avisoValidacion('La firma del representante de Savian es obligatoria', document.getElementById('firmaBox1'));
    return false;
  }

  if (!fS[2]) {
    avisoValidacion('La firma del cliente es obligatoria', document.getElementById('firmaBox2'));
    return false;
  }
...
async function enviarDesdeOverlay() {
  ...
  try {
    await enviarCorreos(_pdfNombre || 'Acta', _pdfBase64);
  } finally {
...
function confirmarReset() {
  ...
  if (localStorage.getItem(BORRADOR_KEY) &&
      confirm('También hay un borrador guardado en este navegador.\n¿Quieres borrarlo también?')) {
    borrarBorrador(false);
  }
}
function resetForm() {
  // Fecha: hoy
  ...
  // Localización
  document.getElementById('loc').value = '';
  if (document.getElementById('responsableFirma')) document.getElementById('responsableFirma').value = '';
...
/* ══════════════════════════════════════════════════
   💾  BORRADOR LOCAL — Guardar / cargar en navegador
══════════════════════════════════════════════════ */
const BORRADOR_KEY = 'borradorActaSavian_v1';

function getFirmaDataUrl(id) {
  const cv = document.getElementById('firma' + id);
  if (!cv || !fS[id] || !cv.width || cv.style.display === 'none') return '';
  return cv.toDataURL('image/png');
}

function restaurarFirmaDesdeDataUrl(id, dataUrl) {
  const cv = document.getElementById('firma' + id);
  const bx = document.getElementById('firmaBox' + id);
  if (!cv || !bx || !dataUrl) {
    limpiarFirma(id);
  ...
  const r = bx.getBoundingClientRect();
  const dpr = window.devicePixelRatio || 2;
  const ancho = r.width || bx.offsetWidth || 300;
  const alto = r.height || bx.offsetHeight || 140;
...
function recogerTrabajadores() {
  return [...document.querySelectorAll('#trabajadores .worker-row')].map(row => {
...
function pintarTrabajadores(trabajadores) {
  const cont = document.getElementById('trabajadores');
  cont.innerHTML = '';
  wc = 0;

  const lista = Array.isArray(trabajadores) && trabajadores.length ? trabajadores : [{}, {}];
  lista.forEach(t => {
    addWorker();
    const row = cont.lastElementChild;
    const inputs = row.querySelectorAll('input');
    if (inputs[0]) inputs[0].value = t.nombre || '';
    if (inputs[1]) inputs[1].value = t.entrada || '';
    if (inputs[2]) inputs[2].value = t.salida || '';
  });
}

function recogerDispositivos() {
  return [...document.querySelectorAll('#deviceRows tr')].map(tr => {
    ...
    return {
      descripcion: textInputs[0]?.value || '',
      unidades: numInput?.value || '',
      entregado: !!checks[0]?.checked,
      retirado: !!checks[1]?.checked,
      detalles: textInputs[1]?.value || ''
    };
  });
}

function pintarDispositivos(dispositivos) {
  ...
  dispositivos.forEach(d => {
    ...
    if (textInputs[0]) textInputs[0].value = d.descripcion || '';
    if (numInput) numInput.value = d.unidades || '';
    if (checks[0]) checks[0].checked = !!d.entregado;
    if (checks[1]) checks[1].checked = !!d.retirado;
    if (textInputs[1]) textInputs[1].value = d.detalles || '';
  });
}

function guardarBorrador(mostrarAviso = true) {
  const datos = {
    ...
    responsableFirma: document.getElementById('responsableFirma')?.value || '',
    tipoActivo,
    trabajos: document.getElementById('trabajos')?.value || '',
    trabajadores: recogerTrabajadores(),
    dispositivos: recogerDispositivos(),
    firmas: {
      firma1: getFirmaDataUrl(1),
      firma2: getFirmaDataUrl(2)
    }
  };

  localStorage.setItem(BORRADOR_KEY, JSON.stringify(datos));
  if (mostrarAviso) showToast('Borrador guardado en este navegador', 'success');
}
...
function activarAutoGuardado() {
  ...
  if (!form) return;
  form.addEventListener('input', autoGuardarBorrador, true);
  form.addEventListener('change', autoGuardarBorrador, true);
}
...
function cargarBorrador() {
  ...
  if (!raw) {
    showToast('No hay ningún borrador guardado en este navegador', 'warn');
    return;
  }
  ...
  try {
    ...
    showToast('El borrador guardado está dañado. Mejor borrarlo y guardar uno nuevo.', 'error');
    return;
  }

  document.getElementById('fecha').value = datos.fecha || hoy;
  document.getElementById('cliente').value = datos.cliente || '';
  document.getElementById('centro').value = datos.centro || '';
  document.getElementById('loc').value = datos.loc || '';
  if (document.getElementById('responsableFirma')) document.getElementById('responsableFirma').value = datos.responsableFirma || '';
  document.getElementById('trabajos').value = datos.trabajos || '';

  poblarCombo('cliente', '');
  poblarCombo('centro', '');
  cerrarCombos();

  pintarTrabajadores(datos.trabajadores);
  pintarDispositivos(datos.dispositivos);

  tipoActivo = '';
  setTipo(datos.tipoActivo || '');

  limpiarFirma(1);
  limpiarFirma(2);
  setTimeout(() => {
    if (datos.firmas?.firma1) restaurarFirmaDesdeDataUrl(1, datos.firmas.firma1);
    if (datos.firmas?.firma2) restaurarFirmaDesdeDataUrl(2, datos.firmas.firma2);
  }, 50);

  showToast('Borrador cargado correctamente', 'success');
}

function borrarBorrador(mostrarAviso = true) {
  localStorage.removeItem(BORRADOR_KEY);
  if (mostrarAviso) showToast('Borrador eliminado', 'success');
}
...
async function completar() {
  ...
  if (btnCompletar) {
    ...
    btnCompletar.classList.add('is-loading');
  }
  ...
  try {
    ...
    if (btnCompletar) {
      ...
      btnCompletar.classList.remove('is-loading');
    }
  }
}
/* ══════════════════════════════════════════════════
