...
function mostrarOverlayPDF(nombre) {
  ...
  const instruccion = esIOS
    ? `<p style="margin:0 0 8px;font-size:13px;opacity:.75;">
        En Safari: pulsa <strong>↑ Compartir</strong> → <strong>"Guardar en Archivos"</strong>
       </p>`
    : '';
  ...
  overlay.innerHTML = `
    <div style="background:#1e1e1e;border-radius:16px;width:100%;max-width:400px;
      ...
      <div style="padding:20px;font-family:'DM Sans',sans-serif;">
        ...
        <!-- Botón guardar / abrir -->
        ${esIOS ? `
        <a href="${_pdfBlobUrl}" target="_blank" rel="noopener"
        ...
        <button onclick="descargarPDFDirecto()"
          style="display:flex;align-items:center;justify-content:center;gap:10px;
                 background:#1a8a7a;color:white;border:none;width:100%;
                 padding:16px;border-radius:12px;font-size:16px;font-weight:700;
                 font-family:'DM Sans',sans-serif;margin-bottom:12px;
...
/* ══════════════════════════════════════════════════
   ✅  BOTÓN COMPLETAR: genera PDF y muestra opciones
══════════════════════════════════════════════════ */
async function completar() {
  ...
  try {
    ...
    // Crear blob URL (funciona en iOS y Android)
    const blob = pdf.output('blob');
    _pdfBlobUrl = URL.createObjectURL(blob);

    setLoading(false);

    // Mostrar overlay con opciones
...
/* ══════════════════════════════════════════════════
   🖨️  BOTÓN IMPRIMIR: igual que completar (mismas opciones)
══════════════════════════════════════════════════ */
