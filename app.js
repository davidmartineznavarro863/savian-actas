...
async function enviarCorreos(nombre, pdfBase64) {
  ...
  `Centro: ${centro}`,
  `Fecha: ${fecha}`,
  `Responsable firma empresa: ${responsable}`,
  `Tecnicos: ${tecnicos}`,
  `Trabajos: ${trabajos}`
].join('\n');
  try {
...
function resetForm() {
  ...
  // Localización y responsable de firma
  document.getElementById('loc').value = '';
  document.getElementById('responsableEmpresa').value = '';
...
function guardarBorrador() {
  const datos = {
    ...
    responsableEmpresa: document.getElementById('responsableEmpresa')?.value || '',
    tipoActivo,
...
function cargarBorrador() {
  ...
  document.getElementById('responsableEmpresa').value = datos.responsableEmpresa || '';
...
function validarFormulario() {
  ...
  const camposObligatorios = [
    ...
    { id: 'centro', nombre: 'Centro de Trabajo' },
    { id: 'loc', nombre: 'Localización Específica' },
    { id: 'responsableEmpresa', nombre: 'Responsable de la empresa que firma' }
  ];
  ...
  camposObligatorios.forEach(campo => {
    ...
    if (!el || !String(el.value || '').trim()) {
      faltan.push(campo.nombre);
      if (el) el.classList.add('campo-error');
    }
  });

  const trabajadoresConNombre = [...document.querySelectorAll('#trabajadores .worker-nombre')]
    .filter(input => String(input.value || '').trim());

  if (trabajadoresConNombre.length < 1) {
    faltan.push('Mínimo un trabajador');
    const primerTrabajador = document.querySelector('#trabajadores .worker-nombre');
    if (primerTrabajador) primerTrabajador.classList.add('campo-error');
  }

  if (faltan.length) {
    const cardDatos = document.getElementById('card-datos');
    const cardTrabajadores = document.getElementById('card-trabajadores');
    if (faltan.some(x => x !== 'Mínimo un trabajador')) cardDatos?.classList.remove('collapsed');
    if (faltan.includes('Mínimo un trabajador')) cardTrabajadores?.classList.remove('collapsed');
    ...
    if (primerError) {
      ...
      setTimeout(() => primerError.focus?.(), 250);
    }
    return false;
  }
...
document.addEventListener('input', (e) => {
  if (e.target.classList?.contains('campo-error') && String(e.target.value || '').trim()) {
