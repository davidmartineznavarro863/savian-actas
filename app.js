...
function selectCliente(val) {
  ...
  // Estilo placeholder
  selCliente.classList.toggle('placeholder-active', !val);
  ...
  // Poblar centros
  const centros = (clientData[val] || {centros:[]}).centros;
  centros.forEach(c => {
    const opt = document.createElement('option');
    opt.value = c.name;
...
function selectCentro(name) {
  const selCentro = document.getElementById('centro');
  selCentro.classList.toggle('placeholder-active', !name);

  const cli = document.getElementById('cliente').value;
  const ct  = (clientData[cli] || {centros:[]}).centros.find(c => c.name === name);
  if (ct && ct.lat !== null) {
    document.getElementById('loc').value = ct.lat + ', ' + ct.lon;
  }
}

// Actualizar color placeholder al cambiar selects
...
function addWorker() {
  ...
  row.innerHTML = `
    <div>
      <div class="worker-num">Trabajador ${wc}</div>
...
async function enviarCorreos(nombre) {
  ...
  // Recoger técnicos
  const filas   = [...document.querySelectorAll('#trabajadores .worker-row')];
...
/* ══════════════════════════════════════════════════
   📥  DESCARGA PDF — compatible iOS / Android / Desktop
══════════════════════════════════════════════════ */
function descargarPDF(pdf, nombre) {
  const esIOS = /iphone|ipad|ipod/i.test(navigator.userAgent);

  if (esIOS) {
    // iOS: mostrar PDF en overlay con iframe (popup bloqueados en Safari async)
    const uri = pdf.output('datauristring');

    // Eliminar overlay anterior si existe
    const prev = document.getElementById('pdf-overlay-ios');
    if (prev) prev.remove();

    const overlay = document.createElement('div');
    overlay.id = 'pdf-overlay-ios';
    ...
    overlay.innerHTML = `
      <div style="background:#1a8a7a;color:white;padding:12px 16px;
                  display:flex;justify-content:space-between;align-items:center;
                  font-family:'DM Sans',sans-serif;flex-shrink:0;">
        <span style="font-size:14px;font-weight:700">📄 ${nombre}.pdf</span>
        <button onclick="document.getElementById('pdf-overlay-ios').remove()"
      ...
      <div style="background:#116658;color:white;padding:8px 16px;text-align:center;
                  font-size:13px;font-family:'DM Sans',sans-serif;flex-shrink:0;">
        Pulsa <strong>↑ Compartir</strong> → <strong>"Guardar en Archivos"</strong>
      </div>
      <iframe src="${uri}"
...
async function completar() {
  ...
  try {
    ...
    // 2. Descargar PDF (compatible iOS)
    descargarPDF(pdf, nombre);

    // 3. Enviar correos
    setLoading(true, 'Enviando correos...');
    await enviarCorreos(nombre);

  } catch (err) {
    console.error(err);
    showToast('❌ Error al generar el PDF o enviar correos', 'error');
...
/* ══════════════════════════════════════════════════
   🖨️  BOTÓN IMPRIMIR: PDF + diálogo impresión
══════════════════════════════════════════════════ */
async function imprimir() {
  ...
  try {
    ...
    setLoading(false);
    const esIOS = /iphone|ipad|ipod/i.test(navigator.userAgent);
    if (!esIOS) setTimeout(()=>window.print(), 300);
  } catch(err) {
    console.error(err);
    showToast('❌ Error al preparar el PDF', 'error');
  } finally {
    setLoading(false);
    document.title='Acta Visita Técnica – Savian';
  }
